...
int main(void) {
	...
	FILE * f_in;
	int id_frag = 0, lastACK = 0, id_lastfrag =  0, oldACK=0, nb_ACK=0, firstprint_SS=0, firstprint_CA=0, currentACK= 0, ACKcounter=0;
				...
				while (1) {
					...
					//if (ssthresh > cwnd){
						...
						if (firstprint_SS ==0){
							...
							printf(ANSI_COLOR_YELLOW "Slowstart..."ANSI_COLOR_RESET"\n");
							firstprint_SS = -1;
						}
						...
						if (flightSize>= cwnd){
							...
							do{
								...
								if (atoi(index(buf, 'K') + 1) == lastACK){
									//printf(ANSI_COLOR_GREEN"Dubliqué!! %d" ANSI_COLOR_RESET"\n", lastACK);
									cwnd = INITIAL_CWND;
									break;
								}
								
								if ((lastACK < atoi(index(buf, 'K') + 1)) && (atoi(index(buf, 'K') + 1) > oldACK))
						...
						if (flightSize>= cwnd){
							...
							do{
								...
								if (lastACK == currentACK){
									ACKcounter++;
									if (ACKcounter == MAX_ACK_DUPLICATED){
										printf(ANSI_COLOR_GREEN"ACK %d dupliqué!"ANSI_COLOR_RESET"\n", lastACK);

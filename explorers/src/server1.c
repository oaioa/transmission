...
int main(int argc, char *argv[]) {
	...
	char *message = (char*) malloc(sizeof(char) * FRAGLEN); //envoy√©
	...
	while (1) {
		...
		if (compare != 0) {
			...
			if ((f_in = fopen(ENTREE, "rb")) == NULL) {	//b because binary
				...
				//printf("\nvery last ACK: %d\n", lastACK);
				printf(ANSI_COLOR_BLUE"\nFichier de taille " ANSI_COLOR_RED" %f" ANSI_COLOR_BLUE" Ko envoy√©\n",(float) len / 1000);
				printf("Dernier RTT: " ANSI_COLOR_RED"%d " ANSI_COLOR_BLUE"¬µsec\n",RTT_microsec);
				printf("DurÈe d'envoi: " ANSI_COLOR_RED"%fsec " ANSI_COLOR_BLUE"\n",delta);
				printf("DÈbit moyen: "ANSI_COLOR_RED "%f Ko/s" ANSI_COLOR_BLUE"\n",	len / (1024 * delta));
				printf("Envoi terminÈ." ANSI_COLOR_RESET"\n\n");
